...
class ECCTest(TestCase):
    ...
    def test_add(self):
        ...
        for elem in additions:
            x_1 = FieldElement(elem[0], prime)
            y_1 = FieldElement(elem[1], prime)
            p_1 = Point(x_1, y_1, a, b)

            x_2 = FieldElement(elem[2], prime)
            y_2 = FieldElement(elem[3], prime)
            p_2 = Point(x_2, y_2, a, b)

            x_3 = FieldElement(elem[4], prime)
            y_3 = FieldElement(elem[5], prime)
            p_3 = Point(x_3, y_3, a, b)
            self.assertEqual(p_1 + p_2, p_3)
...
def exercise_1():
    ...
    b = FieldElement(7, prime)
    def on_curve(x, y):
        field_x = FieldElement(x, prime)
        field_y = FieldElement(y, prime)
        return field_y**2 == field_x**3 + a*field_x + b
...
def exercise_2():
    ...
    for p in points:
        x_1 = FieldElement(p[0][0], prime)
        y_1 = FieldElement(p[0][1], prime)
        p_1 = Point(x_1, y_1, a, b)

        x_2 = FieldElement(p[1][0], prime)
        y_2 = FieldElement(p[1][1], prime)
        p_2 = Point(x_2, y_2, a, b)
...
def exercise_4():
    ...
    # (scalar, x, y)
    scalar_mults = [(2, 192, 105), (2, 143, 98), (2, 47, 71), (4, 47, 71), (8, 47, 71), (21, 47, 71)]
    for elem in scalar_mults:
        x = FieldElement(elem[1], prime)
        y = FieldElement(elem[2], prime)
        p = Point(x, y, a, b)

        scalar = elem[0]
        additions = 1
        product = p
        while additions < scalar:
            product += p
            additions += 1
...
def exercise_5():
    ...
    point = Point(FieldElement(15, prime), FieldElement(86, prime), a, b)
    inf = Point(None, None, a, b)
    counter = 1
    product = point
    while product != inf:
...
def verify_secp256k1():
    ...
    a = FieldElement(0, prime)
    b = FieldElement(7, prime)
    g_x = FieldElement(0x79be667ef9dcbbac55a06295ce870b07029bfcdb2dce28d959f2815b16f81798, prime)
    g_y = FieldElement(0x483ada7726a3c4655da4fbfc0e1108a8fd17b448a68554199c47d08ffb10d4b8, prime)

    on_curve = g_y**2 == g_x**3 + a*g_x + b
    print(on_curve)
...
# s = 0xc7207fee197d27c618aea621406f6bf5ef6fca38681d82b2f06fddbdce6feab6
def exercise_6():
    def is_valid(point, z, r, s):
        s_inv = pow(s, N - 2, N)
        u = z * s_inv
        v = r * s_inv
        return (u*G + v*point).x.num == r
...
def exercise_7():
    ...
    # k = int(random.random() * 2**256)
    k = 1234567890
    r = (k * G).x.num
    k_inv = pow(k, N - 2, N)
    s = (z + r*e) * k_inv % N
    point = e * G
    print(point, hex(z), hex(r), hex(s))
